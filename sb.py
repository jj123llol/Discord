...
to get started install discord.py-self, PyNaCl, and uwuipy
...
async def Update_Settings():
    global prefix, others_allowed, watching, whitelisted, afk, consent, gay, pp,Sniper,autoRJ
    time.sleep(.1)
    settings = {
        "prefix": prefix,
...
global prefix, others_allowed, loaded, watching, whitelisted, spam_ping, cmdlists, bot_runner, afk, consent, pp, gay, Sniper, autoRJ
autoRJ = settings["autorj"]
prefix, others_allowed = settings["prefix"], settings["others_allowed"]
afk, gay, pp, Sniper = settings["afk"],settings["gay"], settings["pp"], settings["Sniper"]
...
@addcmd("ping", 1, "ping")
async def temp(msg, message, self):
...
async def temp(msg, message, seld):
...
async def temp(msg, message, self):
    ...
    try:
        ...
        for file in attach:
            files.append(await file.to_file(filename="temp.gif"))
...
async def temp(msg, message, self):
    ...
    for mention in message.mentions:
        ...
        if mention.display_avatar == None:
            ...
            continue
        avatars.append(await mention.display_avatar.to_file())
    await msg.delete()
    try:
...
async def temp(msg, message, self):
    ...
    for mention in message.mentions:
        if mention.id == 1195825285866717215:
            ...
            if str(mention.id) in pp:
                size = pp[str(mention.id)]
            else:
                size = str(random.randint(0, 50))
                pp[str(mention.id)] = size
                await Update_Settings()
...
async def temp(msg, message, self):
    ...
    for mention in message.mentions:
        if mention.id == 1195825285866717215:
            ...
            if str(mention.id) in gay:
                gaynum = gay[str(mention.id)]
            else:
                gaynum = str(random.randint(0, 100))
                gay[str(mention.id)] = gaynum
                await Update_Settings()
            
        await message.reply(f"<@{mention.id}> is " + gaynum + "% gay!")
...
async def teml(msg, message, self):
...
async def temp(msf, message, self):
    await message.reply("https://www.fortnite.com/item-shop?lang=en-US")
    
    
@addcmd("consent", 2, "consent")
async def temp(msf, message, self):
    global consent
    consent = not consent
    if consent == True:
        await message.reply("Consent prompts turned on")
    else:
        await message.reply("Consent prompts turned off")
    await Update_Settings()
    
@addcmd("snipe", 2, "snipe")
async def temp(msf, message, self):
    global Sniper
    Sniper = not Sniper
    if Sniper == True:
        await message.reply("Sniping gifts!")
    else:
        await message.reply("Stopped sniping!")
    await Update_Settings()
    
@addcmd("autorj", 2, "autorj")
async def temp(msf, message, self):
    global autoRJ
    autoRJ = not autoRJ
    if autoRJ == True:
        await message.reply("will auto rj vc's!")
    else:
        await message.reply("auto rj off!")
    await Update_Settings()
    
    
    
@addcmd("spit",1,"spit @person") 
async def temp(msg, message,self):
    for mention in message.mentions:
        await message.reply(f"<@{message.author.id}> spit on <@{mention.id}> :sweat_drops:") 
    
    
@addcmd("backup", 2, "backup")
async def temp(msf, message, self):
    fails = 0
    passed = 0
    failed_names = []
    ...
    await message.edit("this will take a bit..")
    
    data["bio"] = self.user.bio
    await message.edit("saved bio!")
    
    for friend in self.friends:
        try:
            data["Friends"].append(friend.user.name)
            passed = passed + 1
        except:
            print("failed to save a friend's user!")
            fails = fails +1
    
    await message.edit("Friend usernames backed up!\nbacking up servers..")
    ...
    for guild in self.guilds:
        ...
        for channel in guild.channels:
            ...
            if limit > 15:
                break
            try:
                inv = await channel.create_invite(validate="test")
                inv = inv.url
                data["Servers"].append(inv)
                suceeded = True
                break
            except:
                continue
        if suceeded == False:
            try:
                ...
                suceeded = False
        if suceeded == False:
            print("failed " + guild.name)
            fails = fails + 1
            failed_names.append(guild.name)
        if suceeded == True:
                passed = passed + 1
                print("saved " + guild.name)
                
    await message.edit("server invites backed up..")
    with open("Backed Up Data.json", "w") as f:
        json.dump(data, f, indent=4)
     
    await message.edit(f"Done, failed {str(fails)} tasks, passed {str(passed)} tasks. failed servers printed in console, maybe you dont hsve perms to make invites for them?")
    try:
        os.system('clear')
    except:
        os.system('cls')
    print("Failed Guilds:")
    for guild in failed_names:
        print(guild)
...
class MyClient(discord.Client):
    async def on_voice_state_update(self, member, before, after):
        global autoRJ
        if member == self.user:
            if after.channel == None and autoRJ == True:            
    ...
    async def on_message_delete(self, message):
        ...
        for img in message.attachments:
            att.append(await img.to_file())
        await message.channel.send(f"<@{author.id}> deleted: {content}", files = att)
    ...
    async def on_ready(self):
        ...
        try:
            ...
            os.system('cls')
        print(f"---[[[[ fully loaded, {bot_runner.name}. Made by ant<3 ]]]]---")
        print(str(len(cmds))+" commands")

    async def on_message(self, message):
        global afk, prefix, loaded, others_allowed, whitelisted, Sniper
        content = message.content
        split = content.split(" ")
        for text in split:
            if text.lower().startswith("https://discord.gift/") or text.lower().startswith("discord.gift/"):
                if Sniper == False:
                    continue
                code = "".join(list(text)[text.rindex("/")+1:len(list(text))])
                heads = {
                    "Authorization": token,
                }
                returned = requests.post(f"https://discord.com/api/v9/entitlements/gift-codes/{code}/redeem", headers=heads)
                print(f"Attempted to redeem gift code: {code}\nResponse: {returned.json()["message"]}")
        if len(message.mentions) > 0:
            for mention in message.mentions:
                if str(mention.id) in afk and message.author != self.user:
                   await message.reply(f"<@{mention.id}> is afk for: " + afk[str(mention.id)])
        ...
        try:
            ...
            if cmd in cmds:
                if cmds[cmd]["level"] == 2 and self.user != message.author:
                    await message.reply("missing perms, only the bot owner can use this!")
                    return ""
